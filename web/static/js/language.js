(function ($) {
	...
	function setchartlang (langobj,chartobj) {
		...
		if ( $.type (chartobj) == 'undefined' || chartobj === null ) chartobj = {};
		var flag = true;
		for (key in langobj) {
			var item = key;
	...
	$.fn.cloudLang = function () {
		$.ajax({
			type: 'GET',
			url: window.nps.web_base_url + '/static/page/languages.xml?v=202604292',
			dataType: 'xml',
	...
	$.fn.setLang = function (dom) {
		if ($.type(languages['content']) == 'undefined' || $.type(languages['menu']) == 'undefined') return;
